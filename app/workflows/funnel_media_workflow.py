...
class FunnelMediaWorkflow:
    """Workflow for generating funnel-related media."""
    ...
    async def execute(
        ...
        """Execute funnel media workflow."""
        media_id = str(uuid.uuid4())
        
        try:
            # Determine media types to generate for funnel
            ...
            logger.error("funnel_workflow_error", error=str(e))
            return MediaGenerationResult(
                media_id=media_id,
                success=False,
                error=str(e),
    ...
    def _determine_media_types(self, request: MediaGenerationRequest) -> list[MediaType]:
        """Determine which media types to generate for a funnel."""
        # For funnels, typically generate banners, covers, and thumbnails
        media_types = [MediaType.BANNER, MediaType.COVER]
        
        # Add landing visual if specified
        if request.media_type == MediaType.LANDING_VISUAL:
            media_types.append(MediaType.LANDING_VISUAL)
