...
class CampaignMediaWorkflow:
    """Workflow for generating campaign-related media."""
    ...
    async def execute(
        ...
        """Execute campaign media workflow."""
        media_id = str(uuid.uuid4())
        
        try:
            # Determine media types to generate for campaign
            ...
            logger.error("campaign_workflow_error", error=str(e))
            return MediaGenerationResult(
                media_id=media_id,
                success=False,
                error=str(e),
    ...
    def _determine_media_types(self, request: MediaGenerationRequest) -> list[MediaType]:
        """Determine which media types to generate for a campaign."""
        # For campaigns, typically generate ad creatives, banners, and social graphics
        media_types = [MediaType.AD_CREATIVE, MediaType.BANNER, MediaType.SOCIAL_GRAPHIC]
        
        # Add email graphic if specified
        if request.media_type == MediaType.EMAIL_GRAPHIC:
            media_types.append(MediaType.EMAIL_GRAPHIC)
