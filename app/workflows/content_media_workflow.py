...
class ContentMediaWorkflow:
    """Workflow for generating content-related media."""
    ...
    async def execute(
        ...
        """Execute content media workflow."""
        media_id = str(uuid.uuid4())
        
        try:
            # Determine media types to generate for content
            ...
            logger.error("content_workflow_error", error=str(e))
            return MediaGenerationResult(
                media_id=media_id,
                success=False,
                error=str(e),
    ...
    def _determine_media_types(self, request: MediaGenerationRequest) -> list[MediaType]:
        """Determine which media types to generate for content."""
        # For content, typically generate thumbnails, social graphics, and video thumbnails
        media_types = [MediaType.THUMBNAIL, MediaType.SOCIAL_GRAPHIC]
        
        # Add video thumbnail if specified
        if request.media_type == MediaType.VIDEO_THUMBNAIL:
            media_types.append(MediaType.VIDEO_THUMBNAIL)
