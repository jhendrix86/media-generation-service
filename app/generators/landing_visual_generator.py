...
class LandingVisualGenerator:
    """Generator for landing page visual media."""
    ...
    async def generate(
        ...
        """Generate a landing page visual."""
        enhanced_prompt = self._enhance_prompt(prompt, style, brand_colors)
        
        # Use wide format for landing pages
        landing_size = aspect_ratio if aspect_ratio == "16:9" else "1920x1080"
    ...
    def _enhance_prompt(self, prompt: str, style: Optional[str], brand_colors: Optional[List[str]]) -> str:
        """Enhance prompt for landing visual generation."""
        base_prompt = f"Landing page hero visual: {prompt}"
        ...
        base_prompt += ", high quality, professional, conversion-optimized, clean"
